...
export function useNotes() {
  ...
  const [statistics, setStatistics] = useState<StorageStats | null>(null)
  ...
  const loadTags = async () => {
    const result = await storageService.getTags()
    if (result.success) {
      setTags(result.data || [])
    }
  }
  ...
  const loadDomains = async () => {
    const result = await storageService.getDomains()
    if (result.success) {
  ...
  const loadFolders = async () => {
    const result = await storageService.getFolders()
    if (result.success) {
  ...
  const updateNote = async (id: string, updates: Partial<Omit<Note, 'id' | 'timestamp'>>) => {
    const result = await storageService.updateNote(id, updates)
    if (result.success) {
      await loadNotes()
      await loadStatistics()
    }
  ...
  const updateNoteTags = async (id: string, tags: string[]) => {
    return await updateNote(id, { tags })
  }
  ...
  const exportNotes = async () => {
    return await storageService.exportNotes()
  }

  const importNotes = async (jsonString: string) => {
    const result = await storageService.importNotes(jsonString)
    if (result.success) {
      await loadNotes()
      await loadTags()
      await loadDomains()
